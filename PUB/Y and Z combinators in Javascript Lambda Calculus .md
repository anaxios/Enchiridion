...
- [https://medium.com/swlh/y-and-z-combinators-in-javascript-lambda-calculus-with-real-code-31f25be934ec](https://medium.com/swlh/y-and-z-combinators-in-javascript-lambda-calculus-with-real-code-31f25be934ec)
...
- The fact that **`Y(f) = f(Y(f))`** makes **`Y`** a *fixed-point combinator*. We do not enter the theory of what *fixed-point combinators* are. We just say that they are crucial for the implementation of recursion in Lambda calculus, and we see now why with an example.
...
- fact' = Y(F)
- ```
- `Y` is a *fixed-point combinator*. Y has the property that **`Y(f) = f(Y(f))`**. So also the following holds
- ```
...
- fact’(2) = (F(fact'))(2)
    ...
    - = 2 * fact'(1)
- ```
- See the magic? Just for the fact that `Y` is a *fixed-point combinator* it implements recursion, as long as we pass to it a function in the format of `F`.
...
- But why? The reason is that Javascript adopts a *strict/eager evaluation strategy* while the Y combinator expects a *lazy strategy*.
